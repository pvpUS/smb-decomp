...
void lbl_0000A154(s8 *, struct Sprite *);
...
void lbl_0000A154(s8 *arg0, struct Sprite *sprite)
{
    ...
    if (sprite->bmpId != 0)
        *(u8 **)((u8 *)sprite + 0x2C) = lbl_10000804 + ((s8 *)lbl_801EED98)[sprite->bmpId + 5] * 0x74;
    preview_main(*(u8 **)((u8 *)sprite + 0x2C));
    sp = find_sprite_with_tag(0x22);
    if (sp == NULL)
        return;
    dx = sp->x - pool[32];
    dy = sp->y - pool[14];
    ...
    else if (*(s32 *)&lbl_801EEDA8 != 0x24)
    {
        sprite->mulR = 0x40;
        sprite->mulG = 0x40;
        sprite->mulB = 0x40;
    }
    else if (sprite->userVar == ((s8 *)lbl_801EED98)[5])
    {
        sprite->mulR = 0xFF;
        sprite->mulG = 0xFF;
        sprite->mulB = 0xFF;
