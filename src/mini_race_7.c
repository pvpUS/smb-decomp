/*
 * mini_race.c -- REL module: isolated function lbl_00002018.
...
void lbl_00002018(struct Ball *ball);
...
void lbl_00002018(struct Ball *ball)
{
    ...
    if (st->unk1D0 <= 0x7FFE)
        st->unk1D0++;
    if (active && st->unk1CE > 5)
        st->unk1D0 = 0;
    if (!active && st->unk1D0 > 5)
        st->unk1CE = 0;
}
