 ...
 * byte-matching (part 65 of 94; contiguous .text range).  Each function
...
typedef void (*FightCameraFunc)(struct Camera *, struct Ball *);
extern FightCameraFunc lbl_0001D768[];
...
void lbl_00012248(struct Camera *, struct Ball *);
void lbl_000122C8(struct Camera *, struct Ball *);
...
void lbl_00012248(struct Camera *camera, struct Ball *ball)
{
    if (camera->subState >= 0)
        lbl_0001D768[camera->subState](camera, ball);
}

static void lbl_00012290(struct Camera *camera, struct Ball *ball)
{
    if (!(debugFlags & 0xA))
    {
        camera->subState = 5;
        lbl_000122C8(camera, ball);
    }
}
