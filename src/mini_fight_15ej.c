...
void lbl_00009A68(struct Effect *);
...
void lbl_00009A68(struct Effect *e)
{
    f32 *k = (f32 *)lbl_0001C108;
    f32 c;
    f32 v[3];
    if (polyDisp.flags & 4)
        return;
    mathutil_mtxA_from_translate(&e->pos);
    mathutil_mtxA_rotate_z((s16)e->unk88.z);
    ...
    if (c > k[0])
    {
        ...
        if (v[2] > k[1])
            v[2] = k[1];
        avdisp_set_post_add_color(v[0], v[1], v[2], k[0]);
    }
