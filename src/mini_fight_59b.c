...
void lbl_0001106C(struct Ape *ape, int status);
void lbl_00011270();
...
void lbl_0001106C(struct Ape *ape, int status)
{
    ...
    ape->flags &= -20;
    if (!(sp50.flags & 1) && ball->vel.y < *(f32 *)(cfg + 0xB0))
        ape->flags |= 2;
    else if (mathutil_vec_len(&ball->unkB8) < *(f32 *)(cfg + 0xB4))
        ape->flags |= 1;
    ...
    lbl_00011270(ape, speed);
    ape_skel_anim_main(ape);
    if (!(ape->flags & (1 << 3)))
        func_8003765C(ape);
    ape_face_dir(ape, &ball->lookPoint);
    ball->unk100 = 0;
    ball->lookPointPrio = *(f32 *)(cfg + 0x1C);
}
