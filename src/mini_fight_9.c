 ...
 * byte-matching (part 9 of 94; contiguous .text range).  Each function
...
void lbl_00003CC8(u8 *arg)
{
    struct FightPart *p = (struct FightPart *)(arg + 0x34);
    struct GMAModel *model = minigameGma->modelEntries[20].model;
    int i;

    if ((polyDisp.flags & 4) == 0)
    {
        for (i = 12; i > 0; i--, p++)
        {
            mathutil_mtxA_from_mtxB_translate(&p->pos);
            mathutil_mtxA_rotate_y(p->rotY);
            mathutil_mtxA_rotate_x(p->rotX);
