 ...
 * byte-matching (part 66 of 94; contiguous .text range).  Each function
...
void lbl_000131C4(struct Camera *camera, struct Ball *ball);
...
void lbl_000130AC(struct Camera *camera, struct Ball *ball);
...
void lbl_000130AC(struct Camera *camera, struct Ball *ball)
{
    ...
    camera_clear(camera);
    camera->flags |= 8;
    get_curr_stage_fly_in_position(&sp);
    camera->lookAtStart = sp.pos;
    camera->unk60 = k[26] * sp.radius;
    camera->unk64 = k[27] * sp.radius;
    camera->lookAtEnd = sp.pos;
    camera->unk80 = k[28] * sp.radius;
    *(f32 *)camera->filler84 = k[29] * sp.radius;
    camera->lookAt = camera->lookAtStart;
    camera->rotY = rand() & 0x7FFF;
    camera->flags |= 1;
    camera->unk26 = 1;
    lbl_000131C4(camera, ball);
    camera->subState = 7;
}
