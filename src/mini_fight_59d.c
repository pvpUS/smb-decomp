...
int lbl_00011684(struct Ape *ape, struct Ball *ball);
...
int lbl_00011684(struct Ape *ape, struct Ball *ball)
{
    ...
    int n = lbl_10017664.sub[(s8)ball->playerId].unk4 - 1;
    float t;
    s16 w;
    if (n < 0)
        n = 0;
    ...
    if (!(ape->flags & 0x2000))
    {
        if (ape->unkC2 > w || (ape->flags & 0x4000))
        {
            ape->unk0->u_someDeltaTime = k[26];
            n++;
            ape->flags |= 0x5000;
        }
        else
    ...
    else
    {
        ...
        mathutil_mtxA_rotate_y((s16)(k[50] * (k[51] + k[52] * t)));
