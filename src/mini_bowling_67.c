/*
 * mini_bowling.c -- REL module: isolated function lbl_0000AB98.
...
struct BowlCfg {           // the module's .rodata config block at 0x14800
    u8 filler0[0x3e8];
    f32 zero;              // 0x3e8 == 0.0f
};
struct BowlPin {
...
// lbl_0000AB98 (0xAB98): (re)spawn pin `idx` at `pos` -- reset its flags, clear
// its two scalars, put it at pos with identity orientation and refresh its
// transformed collision points twice.  tbl+0x3e8 is 0.0f.
void lbl_0000AB98(Vec *pos, s8 idx)
{
    struct BowlCfg *cfg = (struct BowlCfg *)lbl_00014800;
    struct BowlPin *pin = &lbl_10018510[idx];

    pin->flags = 3;
    pin->unk178 = 0;
    pin->unk17c = 0;
    pin->unk124 = *pos;
    pin->unk130.x = cfg->zero;
    pin->unk130.y = cfg->zero;
    pin->unk130.z = cfg->zero;
    mathutil_mtx_from_identity(pin->unk148);
    pin->unk13c.x = cfg->zero;
    pin->unk13c.y = cfg->zero;
    pin->unk13c.z = cfg->zero;
    pin->unk180 = 0;
    lbl_0000B848(pin);
    lbl_0000B848(pin);
}
