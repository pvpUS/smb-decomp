/*
 * sel_ngc_rel.c -- REL module: isolated function lbl_00002358.
...
int lbl_00002358(int a, int controllerId);
...
// is `controllerId` the pad that owns the currently active slot?
int lbl_00002358(int a, int controllerId)
{
    int i;
    int count;
    ...
    if (a != 0)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            if (((s32 *)&lbl_801EEDA8)[18 + i] != 0)
                count++;
        }
        if (controllerId == playerControllerIDs[count - 1])
            return 1;
    }
    else
    {
        ...
        {
            if (((s32 *)&lbl_801EEDA8)[18 + i] != 0
             && controllerId == playerControllerIDs[i])
                return 1;
        }
    }
    return 0;
