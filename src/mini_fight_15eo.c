...
void lbl_0000A184(struct Effect *);
...
void lbl_0000A184(struct Effect *effect)
{
    u8 *k = lbl_0001C108;
    struct GMAModel *m;
    Vec v;
    ...
    mathutil_mtxA_rigid_inv_tf_tl(&v);
    if (v.z < *(f32 *)k)
    ...
    avdisp_set_post_mult_color(effect->unk18, effect->unk18, effect->unk18,
                               *(f32 *)(k + 4));
    ...
    mathutil_mtxA_scale_s(*(f32 *)(k + 0x120) * s);
    avdisp_set_bound_sphere_scale(*(f32 *)(k + 0x120) * s);
    avdisp_draw_model_culled_sort_translucent(m);
