/*
 * mini_race.c -- REL module: isolated function lbl_0000B560.
...
// 10 model ids at lbl_00013BD0 + 0x58, copied to the stack wholesale.
...
void lbl_0000B560(struct Ball *ball)
{
    u8 *cfg = lbl_00013BD0;
    struct RaceModelTbl tbl = *(struct RaceModelTbl *)(cfg + 0x58);
    u8 *st = (u8 *)ball->unk144;
    u8 v = 0;
    s16 i;
    s16 model;
    ...
    if (*(s16 *)(st + 0x268) & 8)
        return;
    mathutil_mtxA_from_mtxB_translate_xyz(ball->pos.x,
        *(f32 *)(cfg + 0x6C) + (ball->pos.y + ball->currRadius), ball->pos.z);
    mathutil_mtxA_sq_from_identity();
    mathutil_mtxA_scale_xyz(*(f32 *)(cfg + 0x70), *(f32 *)(cfg + 0x70),
                            *(f32 *)(cfg + 0x70));
    gxutil_load_pos_nrm_matrix(mathutilData->mtxA, 0);
    avdisp_draw_model_culled_sort_translucent(minigameGma->modelEntries[model].model);
}
