/*
 * mini_bowling.c -- REL module: isolated function run lbl_0000DA0C .. lbl_0000E450.
 * This file holds one contiguous run of functions, meant to be a single
 * pure-C file once ALL of them are converted from the asm-includes below.
 * Until the last stub becomes C this file still contains an `asm` block,
 * which disables mwcc's scheduler/peephole for the C functions here -- so
 * convert the whole run (or match function-by-function with --isolate
 * singletons first, then consolidate into this range file).
 * Keep the Makefile SOURCES order so the .text layout is preserved.
 * Generated by tools/rel_split.py -- regenerate rather than hand-editing.
 */

#include <stddef.h>
#include <stdio.h>
#include <stdlib.h>
#include <dolphin.h>
#include "global.h"
#include "ball.h"
#include "bitmap.h"
#include "camera.h"
#include "event.h"
#include "functions.h"
#include "game.h"
#include "info.h"
#include "input.h"
#include "light.h"
#include "mathutil.h"
#include "mode.h"
#include "mot_ape.h"
#include "polydisp.h"
#include "pool.h"
#include "rend_efc.h"
#include "sound.h"
#include "sprite.h"
#include "stage.h"
#include "thread.h"
#include "variables.h"
#include "window.h"
#include "world.h"
#include "pause_menu.h"
#include "string.h"
#include "avdisp.h"
#include "background.h"
#include "obj_collision.h"
#include "ord_tbl.h"
#include "stcoli.h"
#include "shadow.h"
#include "vibration.h"

// Addresses loaded by the code that live in this module's data/rodata/bss
// (defined in asm/mini_bowling.s) or imported.  Declared so mwcc accepts `@ha/@l`.
extern u8 lbl_0000F020[];
extern u8 lbl_00010C40[];
extern u8 lbl_00010CC0[];
extern u8 lbl_00010CC8[];
extern u8 lbl_00010D58[];
extern u8 lbl_00010D60[];
extern u8 lbl_00010EE0[];
extern u8 lbl_00010EE8[];
extern u8 lbl_00010EF0[];
extern u8 lbl_00010F24[];
extern u8 lbl_00010F94[];
extern u8 lbl_00011250[];
extern u8 lbl_00011258[];
extern u8 lbl_000112C0[];
extern u8 lbl_000112C8[];
extern u8 lbl_00011338[];
extern u8 lbl_00011340[];
extern u8 lbl_00011490[];
extern u8 lbl_000146A0[];
extern u8 lbl_000147F8[];
extern u8 lbl_00014800[];
extern u8 lbl_00014B78[];
extern u8 lbl_00014BE8[];
extern u8 lbl_00014C00[];
extern u8 lbl_00014C08[];
extern u8 lbl_00014CB8[];
extern u8 lbl_00014D70[];
extern u8 lbl_00014D90[];
extern u8 lbl_00014E10[];
extern u8 lbl_00014F20[];
extern u8 lbl_00014F24[];
extern u8 lbl_00015020[];
extern u8 lbl_00015024[];
extern u8 lbl_00015040[];
extern u8 lbl_00015380[];
extern u8 lbl_00015388[];
extern u8 lbl_000153E0[];
extern u8 lbl_000153E4[];
extern u8 lbl_000153E8[];
extern u8 lbl_000153F4[];
extern u8 lbl_00015428[];
extern u8 lbl_00015434[];
extern u8 lbl_0001543C[];
extern u8 lbl_00015444[];
extern u8 lbl_00015454[];
extern u8 lbl_00015460[];
extern u8 lbl_0001546C[];
extern u8 lbl_00015480[];
extern u8 lbl_10000000[];
extern u8 lbl_10000004[];
extern u8 lbl_1000000C[];
extern u8 lbl_1000013C[];
extern u8 lbl_1000013E[];
extern u8 lbl_1000013F[];
extern u8 lbl_10000154[];
extern u8 lbl_10000178[];
extern u8 lbl_100004C0[];
extern u8 lbl_100004E0[];
extern u8 lbl_10012140[];
extern u8 lbl_10012180[];
extern u8 lbl_10018510[];

// Imported functions the code calls that no included header declares.
extern void draw_test_camera_target();
extern void func_80071B08();
extern void func_8009C5E4();
extern void func_8009CD5C();
extern void func_8009D794();
extern void func_8009DB40();
extern void func_8009DDC4();
extern void func_800AB2A0();
extern void func_800AB444();
extern void func_800AB6F8();
extern void func_800AC43C();
extern void func_800AC5E0();
extern void mini_commend_free_data();
extern void u_load_minigame_graphics();
extern void u_set_minigame_callbacks_2();

// Forward declarations so mwcc accepts `<fn>@ha/@l` and cross-function
// branches before each function is defined below.
void _prolog(void);
void _epilog(void);
void _unresolved(void);
void lbl_0000020C(void);
void lbl_00000718(void);
void lbl_000009EC(void);
void lbl_00000F98(void);
void lbl_00001888(void);
void lbl_00001908(void);
void lbl_00001B14(void);
void lbl_00001C84(void);
void lbl_00001F1C(void);
void lbl_000021B4(void);
void lbl_00002454(void);
void lbl_000027B0(void);
void lbl_000029A8(void);
void lbl_00002DE0(void);
void lbl_00003574(void);
void lbl_00003A10(void);
void lbl_00003D24(void);
void lbl_00003DC0(void);
void lbl_000042A4(void);
void lbl_00004410(void);
void lbl_000045E8(void);
void lbl_00004A80(void);
void lbl_00004BD8(void);
void lbl_00004D10(void);
void lbl_00004DF8(void);
void lbl_00005128(void);
void lbl_000051E0(void);
void lbl_000054BC(void);
void lbl_00005564(void);
void lbl_00005B0C(void);
void lbl_000066C4(void);
void lbl_000068C4(void);
void lbl_00006E64(void);
void lbl_00006F0C(void);
void lbl_00007518(void);
void lbl_00007650(void);
void lbl_000076D0(void);
void lbl_00007740(void);
void lbl_00007778(void);
void lbl_00007878(void);
void lbl_00007964(void);
void lbl_000079E8(void);
void lbl_00007A6C(void);
void lbl_00007C54(void);
void lbl_00007E74(void);
void lbl_00007FE0(void);
void lbl_000080E0(void);
void lbl_000082E4(void);
void lbl_000086E4(void);
void lbl_0000871C(void);
void lbl_000087CC(void);
void lbl_000089FC(void);
void lbl_00008B8C(void);
void lbl_00008C68(void);
void lbl_00008D2C(void);
void lbl_00008DF0(void);
void lbl_00008EC0(void);
void lbl_00008FB0(void);
void lbl_00009048(void);
void lbl_000090CC(void);
void lbl_00009134(void);
void lbl_0000919C(void);
void lbl_00009230(void);
void lbl_000096B4(void);
void lbl_000097B4(void);
void lbl_00009AA8(void);
void lbl_00009D18(void);
void lbl_00009F60(void);
void lbl_0000A138(void);
void lbl_0000A23C(void);
void lbl_0000A610(void);
void lbl_0000A778(void);
void lbl_0000A808(void);
void lbl_0000A878(void);
void lbl_0000AAAC(void);
void lbl_0000AB98(void);
void lbl_0000AC60(void);
void lbl_0000AD8C(void);
void lbl_0000AF18(void);
void lbl_0000AFEC(void);
void lbl_0000B0AC(void);
void lbl_0000B1BC(void);
void lbl_0000B344(void);
void lbl_0000B460(void);
void lbl_0000B654(void);
void lbl_0000B848(void);
void lbl_0000B914(void);
void lbl_0000BDE0(void);
void lbl_0000BEB8(void);
void lbl_0000C0D0(void);
void lbl_0000C1D0(void);
void lbl_0000CAA8(void);
void lbl_0000D4D4(void);
void lbl_0000D598(void);
void lbl_0000D650(void);
void lbl_0000D7F8(void);
void lbl_0000D8CC(void);
void lbl_0000D90C(void);
void lbl_0000DA0C(u8 *, u8 *);
void lbl_0000DAF4(int, char *);
void lbl_0000DBB8(u8 *, u8 *);
void lbl_0000DD4C(int, char *);
void lbl_0000DE10(u8 *, u8 *);
void lbl_0000DFA4(int timer, int pins);
void lbl_0000E098(u8 *, u8 *);
void lbl_0000E22C(int);
void lbl_0000E2E0(u8 *, u8 *);
void lbl_0000E3A0(int);
void lbl_0000E450(u8 *, u8 *);
void lbl_0000E510(void);
void lbl_0000E5D4(void);
void lbl_0000E7B0(void);
void lbl_0000E870(void);
void lbl_0000E894(void);
void lbl_0000EC38(void);
void lbl_0000EDB0(void);

#pragma force_active on
void lbl_0000DA0C(u8 *done, u8 *st)
{
    u8 *tbl = lbl_00014D70;
    s32 total = *(s32 *)(st + 0x48);
    s32 elapsed = total - *(s16 *)(st + 0x10);
    f32 v;

    v = mathutil_sin((elapsed * 0x8000) / (f32)total);
    *(f32 *)(st + 0x6c) = v;
    if (elapsed > total / 2)
        *(f32 *)(st + 0x40) =
            *(f64 *)(tbl + 0x38) * (*(f64 *)(tbl + 0x40) - *(f64 *)(tbl + 0x48) * v);

    *(s16 *)(st + 0x10) -= 1;
    if (*(s16 *)(st + 0x10) <= 0)
        *done = 0;
}
// lbl_0000DAF4 (0x0000DAF4): spawn the yellow scoreboard banner carrying `text`;
// lbl_0000DBB8 animates it for `timer` frames.
void lbl_0000DAF4(int timer, char *text)
{
    u8 *tbl = lbl_00014D70;
    struct Sprite *sprite = create_sprite();

    if (sprite != NULL) {
        sprite->x = *(f32 *)(tbl + 0x28);
        sprite->y = *(f32 *)(tbl + 0x50);
        sprite->depth = *(f32 *)(tbl + 0x30);
        sprite->mulR = 0xff;
        sprite->mulG = 0xff;
        sprite->mulB = 0;
        sprite->tag = 0x6b;
        sprite->fontId = 9;
        sprite->textAlign = 4;
        sprite->counter = timer;
        sprite->userVar = timer;
        sprite->flags |= 0x1000;
        sprite->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_0000DBB8;
        strcpy(sprite->text, text);
    }
}
void lbl_0000DBB8(u8 *done, u8 *st)
{
    u8 *tbl = lbl_00014D70;
    s16 timer = *(s16 *)(st + 0x10);
    s32 elapsed = *(s32 *)(st + 0x48) - timer;

    if (elapsed < 30) {
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x10) * elapsed;
        *(f32 *)(st + 8) = 120 - (30 - elapsed) * 6;
    } else if (elapsed < 60) {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
        *(f32 *)(st + 8) = *(f32 *)(tbl + 0x50) - *(f32 *)(tbl + 0x54) * mathutil_sin((elapsed - 30) * 0x444);
    } else if (elapsed < 90) {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
        *(f32 *)(st + 8) = *(f32 *)(tbl + 0x50) - *(f32 *)(tbl + 0x58) * mathutil_sin((elapsed - 60) * 0x444);
    } else if (timer <= 15) {
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x60) * timer;
    } else {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
    }

    *(f32 *)(st + 0x40) = *(f32 *)(tbl + 0x34);
    *(f32 *)(st + 0x44) = *(f32 *)(st + 0x40);

    *(s16 *)(st + 0x10) -= 1;
    if (*(s16 *)(st + 0x10) <= 0)
        *done = 0;
}
// lbl_0000DD4C (0x0000DD4C): spawn the red scoreboard banner carrying `text`;
// lbl_0000DE10 animates it for `timer` frames.
void lbl_0000DD4C(int timer, char *text)
{
    u8 *tbl = lbl_00014D70;
    struct Sprite *sprite = create_sprite();

    if (sprite != NULL) {
        sprite->x = *(f32 *)(tbl + 0x28);
        sprite->y = *(f32 *)(tbl + 0x50);
        sprite->depth = *(f32 *)(tbl + 0x30);
        sprite->mulR = 0xff;
        sprite->mulG = 0;
        sprite->mulB = 0;
        sprite->tag = 0x6b;
        sprite->fontId = 9;
        sprite->textAlign = 4;
        sprite->counter = timer;
        sprite->userVar = timer;
        sprite->flags |= 0x1000;
        sprite->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_0000DE10;
        strcpy(sprite->text, text);
    }
}
void lbl_0000DE10(u8 *done, u8 *st)
{
    u8 *tbl = lbl_00014D70;
    s16 timer = *(s16 *)(st + 0x10);
    s32 elapsed = *(s32 *)(st + 0x48) - timer;

    if (elapsed < 30) {
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x10) * elapsed;
        *(f32 *)(st + 8) = 120 - (30 - elapsed) * 6;
    } else if (elapsed < 60) {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
        *(f32 *)(st + 8) = *(f32 *)(tbl + 0x50) - *(f32 *)(tbl + 0x54) * mathutil_sin((elapsed - 30) * 0x444);
    } else if (elapsed < 90) {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
        *(f32 *)(st + 8) = *(f32 *)(tbl + 0x50) - *(f32 *)(tbl + 0x58) * mathutil_sin((elapsed - 60) * 0x444);
    } else if (timer <= 15) {
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x60) * timer;
    } else {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
    }

    *(f32 *)(st + 0x40) = *(f32 *)(tbl + 0x34);
    *(f32 *)(st + 0x44) = *(f32 *)(st + 0x40);

    *(s16 *)(st + 0x10) -= 1;
    if (*(s16 *)(st + 0x10) <= 0)
        *done = 0;
}
// lbl_0000DFA4 (0xDFA4): spawn the "STRIKE!" / "n PINS" banner sprite that
// lbl_0000E098 animates for `timer` frames.
void lbl_0000DFA4(int timer, int pins)
{
    u8 *tbl = lbl_00014D70;
    struct Sprite *sprite;

    if (pins > 0)
    {
        sprite = create_sprite();
        if (sprite != NULL)
        {
            sprite->x = *(f32 *)(tbl + 0x28);
            sprite->y = *(f32 *)(tbl + 0x50);
            sprite->depth = *(f32 *)(tbl + 0x30);
            sprite->mulR = 0xff;
            sprite->mulG = 0x8c;
            sprite->mulB = 0;
            sprite->tag = 0x6b;
            sprite->fontId = 9;
            sprite->textAlign = 4;
            sprite->counter = timer;
            sprite->userVar = timer;
            sprite->flags |= 0x1000;
            sprite->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_0000E098;
            if (pins == 1)
                strcpy(sprite->text, (char *)lbl_0001543C);
            else
                sprintf(sprite->text, (char *)lbl_00015444, pins);
        }
    }
}
void lbl_0000E098(u8 *done, u8 *st)
{
    u8 *tbl = lbl_00014D70;
    s16 timer = *(s16 *)(st + 0x10);
    s32 elapsed = *(s32 *)(st + 0x48) - timer;

    if (elapsed < 30) {
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x10) * elapsed;
        *(f32 *)(st + 8) = 120 - (30 - elapsed) * 6;
    } else if (elapsed < 60) {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
        *(f32 *)(st + 8) = *(f32 *)(tbl + 0x50) - *(f32 *)(tbl + 0x54) * mathutil_sin((elapsed - 30) * 0x444);
    } else if (elapsed < 90) {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
        *(f32 *)(st + 8) = *(f32 *)(tbl + 0x50) - *(f32 *)(tbl + 0x58) * mathutil_sin((elapsed - 60) * 0x444);
    } else if (timer <= 15) {
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x60) * timer;
    } else {
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);
    }

    *(f32 *)(st + 0x40) = *(f32 *)(tbl + 0x34);
    *(f32 *)(st + 0x44) = *(f32 *)(st + 0x40);

    *(s16 *)(st + 0x10) -= 1;
    if (*(s16 *)(st + 0x10) <= 0)
        *done = 0;
}
// lbl_0000E22C (0xE22C): spawn the orange banner sprite that fades out over
// `timer` frames (lbl_0000E2E0 drives its opacity).
void lbl_0000E22C(int timer)
{
    u8 *tbl = lbl_00014D70;
    struct Sprite *sprite = create_sprite();

    if (sprite != NULL) {
        sprite->x = *(f32 *)(tbl + 0x28);
        sprite->y = *(f32 *)(tbl + 0x2c);
        sprite->mulR = 0xff;
        sprite->mulG = 0;
        sprite->mulB = 0x20;
        sprite->tag = 0x6b;
        sprite->fontId = 9;
        sprite->textAlign = 4;
        sprite->counter = timer;
        sprite->userVar = timer;
        sprite->depth = *(f32 *)tbl;
        sprite->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_0000E2E0;
        strcpy(sprite->text, (char *)lbl_00015454);
    }
}
void lbl_0000E2E0(u8 *done, u8 *st)
{
    u8 *tbl = lbl_00014D70;
    s16 timer = *(s16 *)(st + 0x10);
    s32 elapsed = *(s32 *)(st + 0x48) - timer;

    if (timer < 60)
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x68) * timer;
    else if (elapsed < 30)
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x10) * elapsed;
    else
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);

    *(s16 *)(st + 0x10) -= 1;
    if (*(s16 *)(st + 0x10) <= 0)
        *done = 0;
}
// lbl_0000E3A0 (0xE3A0): spawn the yellow banner sprite that fades out over
// `timer` frames (lbl_0000E450 drives its opacity).
void lbl_0000E3A0(int timer)
{
    u8 *tbl = lbl_00014D70;
    struct Sprite *sprite = create_sprite();

    if (sprite != NULL) {
        sprite->x = *(f32 *)(tbl + 0x28);
        sprite->y = *(f32 *)(tbl + 0x2c);
        sprite->mulR = 0xff;
        sprite->mulG = 0xff;
        sprite->mulB = 0;
        sprite->tag = 0x6b;
        sprite->fontId = 9;
        sprite->textAlign = 4;
        sprite->counter = timer;
        sprite->userVar = timer;
        sprite->depth = *(f32 *)tbl;
        sprite->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_0000E450;
        strcpy(sprite->text, (char *)lbl_00015460);
    }
}
// lbl_0000E450 (0xE450): banner fade-out tick -- byte-identical twin of
// lbl_0000E2E0; it shares this TU so both use the one int->float constant.
void lbl_0000E450(u8 *done, u8 *st)
{
    u8 *tbl = lbl_00014D70;
    s16 timer = *(s16 *)(st + 0x10);
    s32 elapsed = *(s32 *)(st + 0x48) - timer;

    if (timer < 60)
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x68) * timer;
    else if (elapsed < 30)
        *(f32 *)(st + 0x6c) = *(f64 *)(tbl + 0x10) * elapsed;
    else
        *(f32 *)(st + 0x6c) = *(f32 *)(tbl + 4);

    *(s16 *)(st + 0x10) -= 1;
    if (*(s16 *)(st + 0x10) <= 0)
        *done = 0;
}
#pragma force_active reset
