/*
 * mini_pilot.c -- REL module: isolated function lbl_000007B8.
...
extern struct PilotItemSpawn lbl_0000C88C[];
...
void lbl_000007B8(void)
{
    ...
    for (i = 0; lbl_0000C88C[i].subType != -1; i++)
    {
        item.pos = lbl_0000C88C[i].pos;
        item.subType = lbl_0000C88C[i].subType;
        item.animGroupId = 0;
        item_create(&item);
    }

    f = ((struct PilotItemSpawn **)lbl_0000CB0C)[*(s16 *)lbl_10000040];
    while (f->subType != -1)
    {
        item.pos = f->pos;
        item.subType = f->subType;
        item.animGroupId = 0;
        item_create(&item);
        ++f;
    }
