/*
 * sel_ngc_rel.c -- REL module: isolated function lbl_0000A67C.
...
void lbl_0000A67C(struct Sprite *sprite);
...
// sprite destFunc: bmpId 0 owns its own preview, otherwise it points into the
// shared preview array at lbl_10000804 (stride 0x74).
void lbl_0000A67C(struct Sprite *sprite)
{
    if (sprite->bmpId == 0)
        preview_free(*(void **)((u8 *)sprite + 0x2C));
    else
        preview_free(lbl_10000804 + (sprite->bmpId - 1) * 0x74);
}
