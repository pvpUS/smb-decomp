...
void lbl_000094F0(struct Effect *);
...
void lbl_000094F0(struct Effect *effect)
{
    struct GMAModel *m = effect->model;

    if (polyDisp.flags & 4)
    {
        if (effect->pos.y < -m->boundSphereRadius)
            return;
        m = ((struct GMAModel **)lbl_10000000)[effect->unkA4];
    }
    mathutil_mtxA_from_mtxB_translate_xyz(effect->pos.x, effect->pos.y,
                                          effect->pos.z);
    ...
    mathutil_mtxA_translate_neg(&m->boundSphereCenter);
