...
void lbl_00013268(struct Effect *e)
{
    ...
    mathutil_mtxA_scale_s(e->scale.x);
    avdisp_set_bound_sphere_scale(e->scale.x);
    avdisp_set_post_mult_color(*(f32 *)(pool + 0x4c), *(f32 *)(pool + 0x4c),
                               *(f32 *)(pool + 0x64), *(f32 *)(pool + 0x4c));
    avdisp_draw_model_unculled_sort_translucent(commonGma->modelEntries[90].model);
    fade_color_base_default();
    avdisp_set_z_mode(1, 3, 1);
}
void lbl_00013318(struct Effect *e)
{
}
void lbl_0001331C(struct Effect *e)
{
    e->timer = 20;
}
