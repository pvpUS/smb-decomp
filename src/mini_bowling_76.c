/*
 * mini_bowling.c -- REL module: isolated function lbl_0000B654.
...
void lbl_0000B654(int slot)
{
    struct BowlSlot *dst = ((struct BowlBank *)lbl_10018510)->s;
    s32 i;
    if (slot < 0) return;
    if (slot > 1) return;
    for (i = 0; i < 10; i++)
        dst[i] = ((struct BowlBank *)(lbl_10018510 + 0xf28))[slot].s[i];
}
