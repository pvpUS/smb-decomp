...
void lbl_00009BA0(struct Effect *);
...
void lbl_00009BA0(struct Effect *effect)
{
    struct Ball *ball;
    f32 r;
    Vec v;
    ...
    if (*(s16 *)(lbl_10017664 + effect->playerId * 0x18 + 0xE) & 8)
    {
        ball = &ballInfo[effect->playerId];
        r = ball->currRadius;
        ...
        if (v.z < -r)
        {
            ...
            v.y = v.y * q;
            v.z = v.z + r;
            mathutil_mtxA_from_translate(&v);
            mathutil_mtxA_scale_s(*(f32 *)lbl_0001C128);
            GXLoadPosMtxImm(mathutilData->mtxA, GX_PNMTX0);
            avdisp_draw_model_culled_sort_translucent(
                commonGma->modelEntries[0x5A].model);
        }
    }
}
