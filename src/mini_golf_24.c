/*
 * mini_golf.c -- REL module: isolated function lbl_000099E0.
...
extern struct GolfPathData lbl_00026AB0;
...
f32 lbl_000099E0(Vec *p)
{
    ...
    struct GolfPathData *t = &lbl_00026AB0;

    besti = 0;
    total = *(f32 *)(k + 0x1FC);
    for (i = 0; i < 11; i++)
    {
        f32 d = mathutil_sqrt(mathutil_sum_of_sq_3(p->x - t->pts[i].x,
                                                   p->y - t->pts[i].y,
                                                   p->z - t->pts[i].z));
    ...
    total = *(f32 *)k;
    for (i = 0; (int)i < besti - 1; i++)
        total += mathutil_sqrt(mathutil_sum_of_sq_3(t->pts[i].x - t->pts[i + 1].x,
                                                    t->pts[i].y - t->pts[i + 1].y,
                                                    t->pts[i].z - t->pts[i + 1].z));
    if (besti != 0)
    {
        total += mathutil_sqrt(mathutil_sum_of_sq_3(t->pts[besti - 1].x - p->x,
                                                    t->pts[besti - 1].y - p->y,
                                                    t->pts[besti - 1].z - p->z));
    }
    else
    {
        total = mathutil_sqrt(mathutil_sum_of_sq_3(t->pts[0].x - p->x,
                                                   *(f32 *)k,
                                                   t->pts[0].z - p->z));
    }
    return total;
