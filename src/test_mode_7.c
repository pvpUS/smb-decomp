/*
 * test_mode.c -- REL module: isolated function lbl_000007AC.
...
// REPEAT_WITH_R_ACCEL(0, btn) with the repeat word hoisted into `rep`.  The
// macro re-reads controllerInfo[0].repeat.button for every button tested;
// across the store to the work pointer mwcc then keeps the ADDRESS
// (&controllerInfo[0].repeat) live instead of the VALUE, costing one addi and
// a reload per test.  Hoisting the read is what the original did.
