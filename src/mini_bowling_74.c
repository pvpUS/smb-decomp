/*
 * mini_bowling.c -- REL module: isolated function lbl_0000B344.
...
struct BowlPin {
    ...
    Vec unk124;                // 0x124 -- pin centre
    Vec unk130;                // 0x130 -- linear velocity
    Vec unk13c;                // 0x13c -- angular velocity
    Mtx unk148;                // 0x148 -- orientation
    u8 filler178[0x184 - 0x178];
};
...
// lbl_0000B344 (0xB344): are all the still-standing pins settled?  A live,
// upright pin that is above the deck must be near-vertical and have both its
// linear and angular velocity below the thresholds at tbl+0x430 / +0x438.
int lbl_0000B344(void)
{
    u8 *tbl = lbl_00014800;
    struct BowlPin *pin;
    ...
    for (i = 0; i < 10; i++, pin++) {
        ...
        if (pin->flags & 2)
            continue;
        if (pin->unk124.y < *(f64 *)(tbl + 0x420))
            continue;
        sp08.x = pin->unk148[0][1];
        sp08.y = pin->unk148[1][1];
        sp08.z = pin->unk148[2][1];
        if (mathutil_vec_dot_prod(&sp14, &sp08) < *(f64 *)(tbl + 0x428)
            && __fabs(pin->unk130.x) + __fabs(pin->unk130.y) + __fabs(pin->unk130.z)
                   < *(f64 *)(tbl + 0x430)
            && __fabs(pin->unk13c.x) + __fabs(pin->unk13c.y) + __fabs(pin->unk13c.z)
                   < *(f64 *)(tbl + 0x438))
            continue;
        ret = 0;
    }
