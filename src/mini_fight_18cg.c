...
void lbl_0000C5FC(void)
{
    ...
    if (debugFlags & 0xA)
        return;
    f = (u32 *)(lbl_10017664 + 0x748);
    if (!(*f & 8))
    {
        modeCtrl.unk18--;
        if (modeCtrl.unk18 % 19 == 0)
        {
            u_play_sound_0(0xFC);
            if (modeCtrl.unk18 != 0)
                lbl_0001A360(0x13);
    ...
    else if (!(*f & 0x10))
    {
        ...
        {
            *f |= 0x10;
            u_play_sound_0(0x1FE);
            u_play_sound_0(0x100D939);
            u_play_sound_0(0xFCD93A);
        }
    }
    oldBall = currentBall;
