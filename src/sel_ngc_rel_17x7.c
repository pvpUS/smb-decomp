...
void lbl_00009E40(s8 *, struct Sprite *);
...
void lbl_00009E40(s8 *arg0, struct Sprite *sprite)
{
    f64 *pool = (f64 *)lbl_00011B68;
    struct Sprite *sp;
    f32 dx;
    f32 dy;

    if (sprite->bmpId == 0 && sprite->userVar != (s8)lbl_801EED88.unk0)
    {
        start_preview_image_read(*(u8 **)((u8 *)sprite + 0x2C), ((u8 *)pool)[(s8)lbl_801EED88.unk0] - 1);
        sprite->userVar = (s8)lbl_801EED88.unk0;
    }
    preview_main(*(u8 **)((u8 *)sprite + 0x2C));
    sp = find_sprite_with_tag(0x21);
    if (sp == NULL)
        return;
    dx = sp->x - pool[24];
    dy = sp->y - pool[25];
    ...
    else if (sprite->userVar == (s8)lbl_801EED88.unk0 && *(s32 *)&lbl_801EEDA8 != 0x1F)
    {
        sprite->mulR = 0xFF;
        sprite->mulG = 0xFF;
        sprite->mulB = 0xFF;
        sprite->addR = (globalAnimTimer << 4) & 0x40;
        sprite->addG = sprite->addR;
        sprite->addB = sprite->addR;
    }
    else if (sprite->userVar == (s8)lbl_801EED88.unk0)
    {
        sprite->mulR = 0xFF;
        sprite->mulG = 0xFF;
        sprite->mulB = 0xFF;
        sprite->addR = pool[26] * (pool[27] - fabs(mathutil_sin(globalAnimTimer << 9)));
        sprite->addG = sprite->addR;
        sprite->addB = sprite->addR;
    }
    else
