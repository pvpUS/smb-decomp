 ...
 * byte-matching (part 63 of 94; contiguous .text range).  Each function
...
void lbl_0001212C(int idx, Point3d *out)
{
    if (idx == 2)
        idx = 3;
    else if (idx == 3)
    ...
    {
        *out = decodedStageLzPtr->startPos[idx].pos;
    }
    else
    {
        *out = ((Point3d *)lbl_0001D734)[idx];
        out->x += decodedStageLzPtr->startPos->pos.x;
        out->y += decodedStageLzPtr->startPos->pos.y;
        out->z += decodedStageLzPtr->startPos->pos.z;
    }
}
