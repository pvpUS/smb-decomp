 ...
 * byte-matching (part 57 of 94; contiguous .text range).  Each function
...
#pragma peephole on
void lbl_00010434(struct Ball *ball)
{
    Point3d startPos;
    Vec pos;
    struct Effect ef;
    ...
    ball->pos.y = *(f32 *)(k + 0x68) + startPos.y;
    ...
    if (ball->ape != NULL)
        ball->ape->flags |= 0x20;
    ball->unk80 = mathutil_vec_distance(&pos, &ball->pos);
    if (ball->unk80 < *(f32 *)(k + 0x6C))
        ball->unk80 = 15;
    else if (ball->unk80 > *(f32 *)(k + 0x70))
        ball->unk80 = 45;
    ball->flags |= 0x200000;
