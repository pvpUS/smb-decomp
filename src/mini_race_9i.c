...
/* @BODY */
// INVENTED -- the module's mode/work block at lbl_10000000.  UNVERIFIED.
struct RaceModeSub
{
    /*0x00*/ u16 unk0;
    /*0x02*/ u16 unk2;
...
{
    /*0x00*/ s32 unk0;
    u8 filler4[0x18 - 0x04];
    /*0x18*/ u8 unk18[0x10];
    /*0x28*/ struct RaceModeSub sub28;
    /*0x38*/ s16 submode;
    u8 filler3A[0x3C - 0x3A];
    /*0x3C*/ s32 timer;
    u8 filler40[0x44 - 0x40];
    /*0x44*/ s16 prevSubmode;
    u8 filler46[0x50 - 0x46];
    /*0x50*/ s16 unk50;
    u8 filler52[0x1068 - 0x52];
    /*0x1068*/ s32 unk1068;
};

// INVENTED -- 4-byte per-course entry table at lbl_00015918.  UNVERIFIED.
struct RaceCourseEnt
{
    /*0x00*/ s16 unk0;
    /*0x02*/ s16 unk2;
};
...
void lbl_0000182C(void)
{
    struct RaceMode *w = (struct RaceMode *)lbl_10000000;
    struct RaceModeSub *s = &w->sub28;

    if (debugFlags & 0xA)
        return;
    if ((s->unk2 & 8) && w->unk50 >= 0 && w->timer == 0x12C)
        lbl_000106C4();
    if (w->timer == 0x1E)
    {
        start_screen_fade(0x101, 0, 0x1E);
        return;
    }
    if (w->timer != 0)
        return;
    ...
    {
        s->unk6 = s->unk6 + 1;
        if (((struct RaceCourseEnt *)lbl_00015918)[s->unk6].unk0 >= 0)
        {
            event_finish_all();
            w->prevSubmode = w->submode;
            w->submode = 1;
            w->unk0 = 0;
        }
        else
        {
            *(u32 *)(pauseMenuState + 4) |= 2;
            w->prevSubmode = w->submode;
            w->submode = 9;
            w->unk0 = 0;
        }
