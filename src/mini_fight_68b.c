...
void lbl_00015300(void)
{
    ...
    if (sp != NULL)
    {
        ...
        sp->textAlign = 4;
        sp->depth = *(f32 *)(k + 8);
        sp->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_0001582C;
        sprintf(sp->text, (char *)(d + 0x4C));
    }
    sp = create_sprite();
    if (sp != NULL)
    {
        ...
        sp->textAlign = 5;
        sp->depth = *(f32 *)(k + 0x10);
        sp->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_000158AC;
        sprintf(sp->text, (char *)(d + 0x58));
        sp2 = create_child_sprite(sp);
        if (sp2 != NULL)
        {
            sp2->x = *(f32 *)(k + 0x14);
            sp2->fontId = 0x65;
            sp2->textAlign = 5;
            sp2->depth = *(f32 *)(k + 0x10);
            sp2->mainFunc = (void (*)(s8 *, struct Sprite *))lbl_00015998;
            sprintf(sp2->text, (char *)(d + 0x5C));
        }
    }
    p = ((s16 **)(d + 0x24))[modeCtrl.playerCount];
    st = g_poolInfo.playerPool.statusList;
    for (i = 0; i < g_poolInfo.playerPool.count; i++, st++, p += 2)
    {
        if (*st == 2)
            lbl_000154B0(p[0], p[1], i);
    }
}
