...
void lbl_0000C06C(void)
{
    ...
    lbl_0001A330(modeCtrl.submodeTimer);
    u_play_sound_0(0x19A);
    lbl_00017DC0(modeCtrl.submodeTimer - 60);
    ball = ballInfo;
    i = g_poolInfo.playerPool.count;
    sub = w->sub;
    for (; i > 0; i--, ball++, sub++)
    {
        if (sub->unk10 > 0 || sub->unk4 < w->unk890 - 1)
        {
            ball->unk148 = 0x28;
            sub->unk12 |= 2;
            unref_func_800393F8(ball);
            lbl_00010018(ball);
            lbl_0000E3A8(ball);
            lbl_10017520.sub[ball->playerId].unk6 = 0;
            worldInfo[ball->playerId * 0x40 + 8] = 4;
            lbl_0000D8B4(ball);
        }
        else
        {
