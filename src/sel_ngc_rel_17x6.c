...
void lbl_00009B94(s8 *, struct Sprite *);
...
void lbl_00009B94(s8 *arg0, struct Sprite *sprite)
{
    ...
    {
        if (*(s32 *)&lbl_801EEDA8 == 0x19 && sprite->userVar != *(s32 *)(w + 0x3C))
        {
            if (sprite->bmpId == 0)
            {
                start_preview_image_read(*(u8 **)((u8 *)sprite + 0x2C), *(s32 *)(w + 0x3C) - 1);
                sprite->userVar = *(s32 *)(w + 0x3C);
            }
            else if (sprite->bmpId - 1 == *(s32 *)lbl_802F1FAC)
            {
                start_preview_image_read(*(u8 **)((u8 *)sprite + 0x2C), *(s32 *)(w + 0x3C) - 1);
                sprite->userVar = *(s32 *)(w + 0x3C);
            }
    ...
    else if (*(s32 *)&lbl_801EEDA8 == 0x19 && sprite->userVar != *(s32 *)(w + 0x3C))
    {
        start_preview_image_read(*(u8 **)((u8 *)sprite + 0x2C), *(s32 *)(w + 0x3C) - 1);
        sprite->userVar = *(s32 *)(w + 0x3C);
    }
    ...
    if (sprite->bmpId == 0)
    {
        ...
        if (modeCtrl.gameType == GAMETYPE_MAIN_COMPETITION)
            ...
            dy = pool[16] + (sp->y - pool[17]);
        sprite->x = pool[18] + dx;
        sprite->y = pool[18] + dy;
    }
    else
    {
        sp = find_sprite_with_tag(0x1C);
        if (sp == NULL)
            return;
        dx = sp->x - pool[14];
        dy = sp->y - pool[19];
