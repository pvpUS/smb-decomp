 ...
 * byte-matching (part 24 of 50; contiguous .text range).  Each function
...
void ball_8003BBF4(struct PhysicsBall *b, Vec *v);
...
void lbl_00006490(struct Ball *ball, struct PhysicsBall *physBall);
...
void lbl_00006490(struct Ball *ball, struct PhysicsBall *physBall)
{
    ...
    if (physBall->flags & 1)
    {
        ...
        {
            if (ball->unk114.y <= *(f64 *)(k + 0x3F8) || ball->pos.y <= *(f64 *)(k + 0x400))
                *(s16 *)lbl_802F1FF4 = 15;
        }
        if (*(u32 *)lbl_802F1FD0 & 0x20)
        {
            t = ball->vel;
            ball_8003BBF4(physBall, &t);
            v = mathutil_sqrt(mathutil_vec_sq_len(&t));
            if (v < *(f64 *)(k + 0x3B8))
                *(u16 *)(m + 0x1C) += 1;
            if (v < *(f64 *)(k + 0x240) || *(u16 *)(m + 0x1C) > 0x3C)
                ball->state = 0;
        }
    }
    else
