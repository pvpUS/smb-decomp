 ...
 * byte-matching (part 18 of 94; contiguous .text range).  Each function
...
void lbl_0000D890(struct Ball *);
void lbl_0000D8B4(struct Ball *);
...
void lbl_0001A348(s32);
void lbl_0001A360(s32);
...
void lbl_0000C06C(void);
void lbl_0000C22C(void);
void lbl_0000C2C8(void);
...
void lbl_0000C2C8(void)
{
    ...
    lbl_0000DF9C();
    lbl_0000E458();
    if (*(u32 *)lbl_801EED98 & 1)
        lbl_0001BA8C();
    modeCtrl.unk18 = 0x68;
    lbl_0001A360(7);
    w->unk88C &= ~0x18;
    n = g_poolInfo.playerPool.count;
    oldBall = currentBall;
    statusA = g_poolInfo.playerPool.statusList;
    ballA = ballInfo;
    for (iA = 0; iA < n; iA++, ballA++, statusA++)
    {
        if (*statusA == STAT_NORMAL)
        {
            currentBall = ballA;
            cameraInfo[ballA->playerId].subState = 2;
        }
    }
    currentBall = oldBall;
    ballB = ballInfo;
    statusB = g_poolInfo.playerPool.statusList;
    for (iB = 0; iB < g_poolInfo.playerPool.count; iB++, ballB++, statusB++)
    {
        if (*statusB == STAT_NORMAL)
        {
            currentBall = ballB;
            unref_func_800393F8(ballB);
            *(s16 *)((u8 *)w->sub + ballB->playerId * 0x18 + 6) = 0;
            ballB->flags &= ~0x180010;
            if (ballB->ape != NULL)
                ballB->ape->flags &= ~0x20;
        }
    }
    currentBall = oldBall;
    statusC = g_poolInfo.playerPool.statusList;
    ballC = ballInfo;
    for (iC = 0; iC < g_poolInfo.playerPool.count; iC++, ballC++, statusC++)
    {
        if (*statusC == STAT_NORMAL)
        {
            currentBall = ballC;
            ballC->unk148 = 5;
        }
    ...
    currentBall = oldBall;
    statusD = g_poolInfo.playerPool.statusList;
    ballD = ballInfo;
    for (iD = 0; iD < g_poolInfo.playerPool.count; iD++, ballD++, statusD++)
    {
        if (*statusD == STAT_NORMAL)
        {
            currentBall = ballD;
            ballD->flags |= 0x400;
            if (*(s16 *)((u8 *)w->sub + ballD->playerId * 0x18 + 0x10) == 0)
            {
                ballD->flags |= 0x1100;
                lbl_0000D890(ballD);
                ballD->winStreak = 3;
    ...
    currentBall = oldBall;
    statusE = g_poolInfo.playerPool.statusList;
    wi = worldInfo;
    for (iE = 0; iE < g_poolInfo.playerPool.count; iE++, wi += 0x40, statusE++)
    {
        if (*statusE == STAT_NORMAL)
        {
            wi[8] = 4;
            *(s16 *)(wi + 0) = 0;
            *(s16 *)(wi + 2) = 0;
    ...
    lbl_00017230();
    lbl_0001A348(modeCtrl.unk18 + 0x13);
    u_play_music(0, 8);
}
