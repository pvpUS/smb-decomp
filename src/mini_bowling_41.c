/*
 * mini_bowling.c -- REL module: isolated function lbl_0000D598.
...
int lbl_0000D598(f32 *, Vec *, Vec *, Vec *, Vec *, f32, f32);
void lbl_0000D650(f32 *, f32 *, Vec *, Vec *, Vec *, Vec *);
...
// lbl_0000D598 (0xD598): swept sphere-vs-sphere test.  lbl_0000D650 returns the
// squared distances at the segment ends; if the pair got closer than the summed
// radii, report the penetration depth and return 1.
int lbl_0000D598(f32 *depth, Vec *a0, Vec *a1, Vec *b0, Vec *b1, f32 ra, f32 rb)
{
    f32 sp28;
    f32 sp24;
    f32 sum;
    ...
    lbl_0000D650(&sp28, &sp24, a0, a1, b0, b1);
    sum = ra + rb;
    sq = sum * sum;
    if (sp28 < sq) {
        if (sp24 < sq)
            *depth = sum - mathutil_sqrt(sp24);
        else
            *depth = *(f32 *)lbl_00014BE8;
        return 1;
    }
    *depth = *(f32 *)lbl_00014BE8;
    return 0;
