...
void lbl_0000B720(void)
{
    ...
    int i;
    struct Ball *ball2;
    struct Ball *oldBall2;
    s8 *status2;
    int i2;
    struct Ball *oldBall3;
    struct Ball *ball3;
    s8 *status3;
    int i3;

    *(s16 *)(lbl_10017664 + 2) = 9;
    modeCtrl.submodeTimer = 0x3C;
    wi = worldInfo;
    status = g_poolInfo.playerPool.statusList;
    for (i = 0; i < g_poolInfo.playerPool.count; i++, wi += 0x40, status++)
    {
        if (*status == STAT_NORMAL)
            wi[8] = 1;
    }
    oldBall2 = currentBall;
    status2 = g_poolInfo.playerPool.statusList;
    ball2 = ballInfo;
    for (i2 = 0; i2 < g_poolInfo.playerPool.count; i2++, ball2++, status2++)
    {
        if (*status2 == STAT_NORMAL)
        {
            currentBall = ball2;
            ball2->flags |= BALL_FLAG_16;
        }
    }
    currentBall = oldBall2;
    *(u32 *)infoWork &= ~8;
    u_play_sound_0(0xD8FC);
    u_play_sound_0(0x183);
    oldBall3 = currentBall;
    status3 = g_poolInfo.playerPool.statusList;
    ball3 = ballInfo;
    for (i3 = 0; i3 < g_poolInfo.playerPool.count; i3++, ball3++, status3++)
    {
        if (*status3 == STAT_NORMAL)
        {
            currentBall = ball3;
            lbl_802F1DFC = ball3->ape->charaId;
            u_somePlayerId = ball3->playerId;
            u_play_sound_0(0x1E);
        }
    }
    currentBall = oldBall3;
}
