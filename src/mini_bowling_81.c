/*
 * mini_bowling.c -- REL module: isolated function lbl_0000C0D0.
...
struct BowlPin {
    u32 flags;                 // 0x000
    Vec pts[24];               // 0x004 -- per-pin collision points
    Vec unk124;                // 0x124 -- pin centre
    u8 filler130[0x184 - 0x130];
};
struct BowlPinPt {
    u8 filler0[0x10];
    f32 unk10;                 // 0x10
...
void lbl_0000C0D0(struct BowlPin *pin, int idx, Vec *axis, Vec *out);
...
// lbl_0000C0D0 (0xC0D0): velocity of collision point `idx` of `pin` under the
// angular velocity `axis`, scaled per-component.
void lbl_0000C0D0(struct BowlPin *pin, int idx, Vec *axis, Vec *out)
{
    u8 *tbl = lbl_00014800;
    Vec sp24;
    ...
    sp18.x = pin->pts[idx].x - pin->unk124.x;
    sp18.y = pin->pts[idx].y - pin->unk124.y;
    sp18.z = pin->pts[idx].z - pin->unk124.z;
    mathutil_vec_cross_prod(&sp18, axis, &sp24);
    s = *(f64 *)(tbl + 0x4c8)
        + *(f64 *)(tbl + 0x4d0) / (*(struct BowlPinPt **)(tbl + 0x3a8))[idx].unk10;
    sp24.x = sp24.x * (*(f32 *)(tbl + 0x388) * s);
    sp24.y = sp24.y * (*(f32 *)(tbl + 0x38c) * s);
    sp24.z = sp24.z * (*(f32 *)(tbl + 0x390) * s);
    *out = sp24;
}
