/*
 * sel_ngc_rel.c -- REL module: isolated function lbl_0000A3A4.
...
void lbl_0000A3A4(struct Sprite *sprite);
...
// sprite drawFunc: preview blit, gated on the current menu page.
void lbl_0000A3A4(struct Sprite *sprite)
{
    if (sprite->bmpId == 0)
    {
        if (*(u32 *)lbl_801EEDA8.filler0 - 23 <= 1)
            return;
    ...
    else
    {
        if (*(u32 *)lbl_801EEDA8.filler0 - 23 <= 1)
            return;
        if (sprite->bmpId - 1 >= *(s32 *)lbl_802F1FAC)
            return;
        if (sprite->bmpId - 1 >= *(s32 *)lbl_802F1FB0)
            return;
    }
