...
// NOTE: src/sound.h declares SoundVol(u16,u8) / SoundPan(u16,u8,u8) /
// SoundPitch(u16,u16).  This TU passes those arguments untruncated (and a
// negative pan offset), so it must have seen plain int parameters; the DOL
// header form cannot be changed without perturbing every other module.
...
// lbl_0000A778 (0xA778): start a positional one-shot sound -- the pan comes
// from the x coordinate at *pos scaled by the double at lbl_000147F8.
void lbl_0000A778(int sndId, f32 *pos, int vol, int pitch)
{
    int h;
    int pan;

    pan = *(f64 *)lbl_000147F8 * *pos;
    h = u_play_sound_1_dupe(sndId);
    SoundVol(h, vol + 0x64);
    SoundPan(h, pan, -63);
    SoundPitch(h, pitch);
}
