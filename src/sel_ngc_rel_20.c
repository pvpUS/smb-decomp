/*
 * sel_ngc_rel.c -- REL module: isolated function lbl_0000A544.
...
void lbl_0000A544(struct Sprite *sprite);
...
// sprite drawFunc: preview blit, suppressed for the still-loading slot.
void lbl_0000A544(struct Sprite *sprite)
{
    if (sprite->bmpId == 0)
    {
        if (*(s32 *)lbl_100000C0 == 0
         && stageInfo.u_stageTimer > *(f64 *)lbl_00011C40
         && sprite->userVar == *(s8 *)(lbl_801EED98 + 5)
         && *(s32 *)&lbl_801EEDA8 == 0x24)
            return;
    }
    else if (sprite->bmpId - 1 >= *(s8 *)(lbl_801EED98 + 9))
        return;

    preview_draw(*(void **)((u8 *)sprite + 0x2C),
                 (sprite->mulR << 16) | (sprite->mulG << 8) | sprite->mulB,
                 (sprite->addR << 16) | (sprite->addG << 8) | sprite->addB,
                 sprite->x, sprite->y, sprite->depth, sprite->scaleX,
                 sprite->scaleY);
}
