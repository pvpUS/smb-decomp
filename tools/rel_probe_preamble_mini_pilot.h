/*
 * MINI_PILOT probe preamble -- 265 lines, 104 externs.  VERIFIED WORKING in
 * run 15 (mini_pilot used it for every rel_pcmp/rel_probe measurement).
 *
 * WHY THIS FILE EXISTS UNDER A MODULE-SUFFIXED NAME.  tools/rel_probe_preamble.h
 * is mini_bowling's.  Run 14's mini_pilot agent regenerated a working one and
 * reported it as left in the tree for the next agent -- but it only ever
 * existed in that agent's WARM COPY, and warm_reset_run<N>.sh copies tools/
 * FROM the main tree, so the next run destroyed it.  Run 15's mini_pilot agent
 * then found the tree's copy broken (130 lines, missing lbl_10000074,
 * lbl_0000D314 and lbl_0000D320 -- every probe touching them died with
 * `undefined identifier`) and rebuilt it from scratch.
 *
 * NOTHING AN AGENT LEAVES IN tools/ SURVIVES THE NEXT RUN'S WARM RESET.  Hand
 * such artifacts to the orchestrator, or they are lost.  That is why this one
 * is committed to the main tree under a name that cannot collide.
 *
 * No tool reads this file programmatically -- paste it into your probe unit.
 */
/*
